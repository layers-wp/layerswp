...
!function(a) {
	...
	b.prototype.load = function(b) {
		for (var c in b)
			if (b.hasOwnProperty(c)) {
				if (this[c])
					continue;if (a.FE.PLUGINS[c] && this.opts.pluginsEnabled.indexOf(c) < 0)
					continue;if (this[c] = new b[c](this),
	...
	b.prototype.destroy = function() {
		if (this.edit.isDisabled())
			return !1;
		if (this.shared.count--,
		this.events.$off(),
		this.events.trigger("destroy"),
		this.events.trigger("shared.destroy"),
		0 === this.shared.count)
			for (var b in this.shared)
				this.shared.hasOwnProperty(b) && delete this.shared[b];
	...
	a.FE.MODULES.node = function(b) {
		...
		function e(e, f) {
			if (a(e).find("table").length > 0)
				return !1;
			if (e.querySelectorAll(a.FE.VOID_ELEMENTS.join(",")).length - e.querySelectorAll("br").length)
				return !1;
			if (e.querySelectorAll(b.opts.htmlAllowedEmptyTags.join(",")).length)
				return !1;
			if (e.querySelectorAll(a.FE.BLOCK_TAGS.join(",")).length > 1)
				return !1;
			if (e.querySelectorAll(b.opts.htmlDoNotWrapTags.join(",")).length)
				return !1;
			var g = c(e);
		...
		function g(c, e, f) {
			...
			e.push(b.$el.get(0)),
			e.indexOf(c.parentNode) >= 0 || c.parentNode && a(c.parentNode).hasClass("fr-inner") || c.parentNode && a.FE.SIMPLE_ENTER_TAGS.indexOf(c.parentNode.tagName) >= 0 && f)
				return null ;
			for (; e.indexOf(c.parentNode) < 0 && c.parentNode && !a(c.parentNode).hasClass("fr-inner") && (a.FE.SIMPLE_ENTER_TAGS.indexOf(c.parentNode.tagName) < 0 || !f) && (!d(c) || !d(c.parentNode) || !f); )
				c = c.parentNode;
			return c
		}
		function h(a) {
	...
	a.FE.MODULES.clean = function(b) {
		...
		function e(a) {
			...
			a = a.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, function(a) {
				return H.push(a),
				"[FROALA.EDITOR.SCRIPT " + (H.length - 1) + "]"
			}),
		...
		function g(a) {
			var b;
			for (b in a)
				a.hasOwnProperty(b) && (b.match(G) || delete a[b]);
			for (var c = "", d = Object.keys(a).sort(), e = 0; e < d.length; e++)
				b = d[e],
				c += a[b].indexOf('"') < 0 ? " " + b + '="' + a[b] + '"' : " " + b + "='" + a[b] + "'";
			return c
		}
		function h(a, c, d) {
			if (b.opts.fullPage) {
		...
		function l() {
			...
			if (c.length) {
				b.selection.save();
				for (var d = 0; d < c.length; d++)
					"" === b.node.attributes(c[d]) && a(c[d]).replaceWith("<" + a.FE.HTML5Map[c[d].tagName] + ">" + c[d].innerHTML + "</" + a.FE.HTML5Map[c[d].tagName] + ">");
				b.selection.restore()
			}
		}
		function m(c) {
			if ("PRE" == c.tagName && o(c),
			c.nodeType == Node.ELEMENT_NODE && (c.getAttribute("data-fr-src") && c.setAttribute("data-fr-src", b.helpers.sanitizeURL(c.getAttribute("data-fr-src"))),
			c.getAttribute("href") && c.setAttribute("href", b.helpers.sanitizeURL(c.getAttribute("href"))),
		...
		function q() {
			for (var c = b.$el.get(0).querySelectorAll("blockquote + blockquote"), d = 0; d < c.length; d++) {
				var e = c[d];
				b.node.attributes(e) == b.node.attributes(e.previousSibling) && (a(e).prev().append(a(e).html()),
				a(e).remove())
			}
		}
		function r() {
			for (var a = b.$el.get(0).querySelectorAll("tr"), c = 0; c < a.length; c++)
				if (a[c].querySelectorAll("th").length) {
					for (var d = a[c]; d && "TABLE" != d.tagName && "THEAD" != d.tagName; )
						d = d.parentNode;
					var e = d;
					"THEAD" != e.tagName && (e = b.doc.createElement("THEAD"),
		...
		function t() {
			...
			if (c)
				for (var d = b.$el.get(0).querySelectorAll("td > " + c + ", th > " + c), e = 0; e < d.length; e++)
					"" === b.node.attributes(d[e]) && a(d[e]).replaceWith(d[e].innerHTML + "<br>")
		}
		function u() {
			r(),
			s(),
		...
		function v() {
			...
			do {
				...
				a = [];
				for (var g = b.$el.get(0).querySelectorAll("li"), h = 0; h < g.length; h++)
					c(g[h]) && a.push(g[h])
			} while (a.length > 0)
		}
		function w() {
		...
		function x() {
			var a, c = function(b) {
				0 === b.querySelectorAll("LI").length && (a = !0,
				b.parentNode.removeChild(b))
			}
			;
			do {
				a = !1;
				for (var d = b.$el.get(0).querySelectorAll("li:empty"), e = 0; e < d.length; e++)
					d[e].parentNode.removeChild(d[e]);
				for (var f = b.$el.get(0).querySelectorAll("ul, ol"), e = 0; e < f.length; e++)
					c(f[e])
			} while (a === !0)
		}
		function y() {
		...
		function A() {
			for (var c = b.$el.get(0).querySelectorAll("li > ul, li > ol"), d = 0; d < c.length; d++) {
				...
				if (b.node.isFirstSibling(e))
					...
					for (var f = e.previousSibling.previousSibling; f && a(f).hasClass("fr-marker"); )
						f = f.previousSibling;
		...
		function B() {
			for (var c = b.$el.get(0).querySelectorAll("li:empty"), d = 0; d < c.length; d++)
				a(c[d]).remove()
		}
		function C() {
			v(),
	...
	a.FE.MODULES.events = function(b) {
		...
		function l(c) {
			...
			return b.edit.isDisabled() || d.hasClass("fr-disabled") ? (c.preventDefault(),
			!1) : "mousedown" === c.type && 1 !== c.which ? !0 : (b.helpers.isMobile() || c.preventDefault(),
		...
		function m(c, d) {
			...
			if ("touchmove" != c.type) {
				...
				c.preventDefault(),
				!e.hasClass("fr-selected"))
					return a(".fr-selected").removeClass("fr-selected"),
					!1;
				if (a(".fr-selected").removeClass("fr-selected"),
		...
		function q(a, c, d) {
			...
			r("window.mouseup", function() {
				a.find(c).removeClass("fr-selected"),
				n()
			})
		}
		function r(a, c, d) {
		...
		function u() {
			t(D, b.id),
			0 == b.shared.count && t(b.shared.$_events, b.sid)
		}
		function v(c, d, e) {
	...
	a.FE.MODULES.markers = function(b) {
		...
		function d(d, e, f) {
			try {
				...
				g.insertNode(c(e, f)),
				e === !0 && d.collapsed)
					for (var h = b.$el.find('span.fr-marker[data-type="true"][data-id="' + f + '"]'), i = h.get(0).nextSibling; i && i.nodeType === Node.TEXT_NODE && 0 === i.textContent.length; )
						a(i).remove(),
						i = h.nextSibling;
				if (e === !0 && !d.collapsed) {
					...
					if (i && i.nodeType === Node.ELEMENT_NODE && b.node.isBlock(i)) {
						...
						do
							i = j[0],
							j = b.node.contents(i);
						while (j[0] && b.node.isBlock(j[0]));a(i).prepend(a(h))
					}
				}
				if (e === !1 && !d.collapsed) {
					...
					if (i && i.nodeType === Node.ELEMENT_NODE && b.node.isBlock(i)) {
						...
						do
							i = j[j.length - 1],
							j = b.node.contents(i);
						while (j[j.length - 1] && b.node.isBlock(j[j.length - 1]));a(i).append(a(h))
					}
	...
	a.FE.MODULES.selection = function(b) {
					...
					if (h.nodeType == Node.ELEMENT_NODE) {
						...
						if (h.childNodes.length > 0 && h.childNodes[g.startOffset]) {
							for (var j = h.childNodes[g.startOffset]; j && j.nodeType == Node.TEXT_NODE && 0 == j.textContent.length; )
								j = j.nextSibling;
							j && j.textContent.replace(/\u200B/g, "") === c().replace(/\u200B/g, "") && (h = j,
							i = !0)
				...
				if (f.rangeCount) {
					...
					if (h.nodeType == Node.ELEMENT_NODE) {
						...
						if (h.childNodes.length > 0 && h.childNodes[g.endOffset] && a(h.childNodes[g.endOffset]).text() === c())
							...
							j.nodeType == Node.ELEMENT_NODE && j && j.textContent.replace(/\u200B/g, "") === c().replace(/\u200B/g, "") && (h = j,
							i = !0)
						}
			...
			if (t() && f.rangeCount)
				for (var g = e(), h = 0; h < g.length; h++) {
					...
					for (var n = [], o = k; o !== l && o !== b.$el.get(0); )
						...
						n.push(o)),
						b.node.isBlock(o) && n.indexOf(o) < 0 && c.indexOf(o) < 0 && c.push(o);
				...
				if (k.length > 0) {
					...
					try {
						...
						if (i.nextSibling == k || k.nextSibling == i) {
							...
							if (t && b.node.isVoid(t) && (t = null ),
							u && b.node.isVoid(u) && (u = null ),
							t && u && t.nodeType == Node.TEXT_NODE && u.nodeType == Node.TEXT_NODE) {
								...
								a(k).remove(),
								b.html.normalizeSpaces(u),
								l = a(b.doc.createTextNode("\u200b")),
								a(u).before(l),
								j.setStart(u, 0),
								j.setEnd(u, 0),
								n = !0) : !u && t && t.nodeType == Node.TEXT_NODE && (a(i).remove(),
								a(k).remove(),
								b.html.normalizeSpaces(t),
								l = a(b.doc.createTextNode("\u200b")),
								a(t).after(l),
						...
						if (!n) {
							...
							if (b.browser.chrome && i.nextSibling == k)
								...
								y && y.style && "block" == b.win.getComputedStyle(y).display ? (j.setEndAfter(y),
								j.setStartAfter(y)) : (w = m(i, j, !0) || a(i).before(l) && j.setStartBefore(i),
		...
		function m(c, d, e) {
			...
			if (f && g && f.nodeType == Node.TEXT_NODE && g.nodeType == Node.TEXT_NODE) {
				...
				return e ? (g.textContent = f.textContent + g.textContent,
				a(f).remove(),
				a(c).remove(),
				b.html.normalizeSpaces(g),
				function() {
					d.setStart(g, h)
				}
			...
			if (f && !g && f.nodeType == Node.TEXT_NODE) {
				var h = f.textContent.length;
				return e ? (b.html.normalizeSpaces(f),
				function() {
					d.setStart(f, h)
				}
				) : (b.html.normalizeSpaces(f),
			...
			return g && !f && g.nodeType == Node.TEXT_NODE ? e ? (b.html.normalizeSpaces(g),
			function() {
				d.setStart(g, 0)
			}
			) : (b.html.normalizeSpaces(g),
		...
		function q() {
			if (o())
				return !1;
			b.$el.find("td").prepend('<span class="fr-mk">' + a.FE.INVISIBLE_SPACE + "</span>"),
			b.$el.find("img").append('<span class="fr-mk">' + a.FE.INVISIBLE_SPACE + "</span>");
			var c = !1
			  , d = p(b.$el.get(0));
		...
		function s(c, d) {
			...
			for (var f = 0; f < e.length; f++) {
				...
				a(g).attr("data-del-cell", !0)) : a(g).find(".fr-marker").length > 0 && (d = s(a(g), d))
			}
			return d
		}
		function t() {
		...
		function u() {
			...
			for (var h = b.$el.find('.fr-marker[data-type="false"]'), f = 0; f < h.length; f++) {
				...
				i.parentNode && b.node.isBlock(i.parentNode) && b.node.isEmpty(i.parentNode) && a(i.parentNode).after(i)
			}
			if (n()) {
				s(b.$el, 0);
						...
						if (q) {
							if (p && p == q)
								...
								r == t)
									...
									u ? (a(u).after(a(t).html()),
									a(t).remove()) : 0 == a(t).parentsUntil(b.$el, "table").length && (a(m).next().after(a(t).html()),
									a(t).remove())
								} else if (null  == t && 0 == a(r).parentsUntil(b.$el, "table").length) {
									for (var p = r; !p.nextSibling && p.parentNode != b.$el.get(0); )
										p = p.parentNode;
									...
									0 == a(r).parentsUntil(b.$el, "table").length && 0 == a(t).parentsUntil(b.$el, "table").length && (a(r).append(a(t).html()),
									a(t).remove())
							}
						} else
		...
		function x(c) {
			for (var d = c.previousSibling; d && d.nodeType == Node.TEXT_NODE && 0 == d.textContent.length; )
				d = d.previousSibling;
			return d ? (b.node.isBlock(d) ? w(d) : "BR" == d.tagName ? a(d).before(a.FE.MARKERS) : a(d).after(a.FE.MARKERS),
			!0) : !1
		}
		function y(c) {
			for (var d = c.nextSibling; d && d.nodeType == Node.TEXT_NODE && 0 == d.textContent.length; )
				d = d.nextSibling;
			return d ? (b.node.isBlock(d) ? v(d) : a(d).before(a.FE.MARKERS),
			!0) : !1
		}
		return {
			text: c,
	...
	a.FE.BLOCK_TAGS = ["p", "div", "h1", "h2", "h3", "h4", "h5", "h6", "pre", "blockquote", "ul", "ol", "li", "table", "td", "th", "thead", "tfoot", "tbody", "tr", "hr", "dl", "dt", "dd", "form"],
	a.extend(a.FE.DEFAULTS, {
	...
	a.FE.MODULES.html = function(b) {
		...
		function g(c) {
			...
			do {
				...
				for (var g = 0; g < e.length; g++)
					(0 === e[g].attributes.length || "undefined" != typeof e[g].getAttribute("href")) && (a(e[g]).remove(),
					f = !0);
				e = b.$el.get(0).querySelectorAll("*:empty:not(" + d.join("):not(") + "):not(.fr-marker)")
			} while (e.length && f)
		}
		function h(d, e) {
			...
			f)
				for (var g = b.node.contents(d.get(0)), h = null , i = 0; i < g.length; i++) {
					var j = g[i];
					if (j.nodeType == Node.ELEMENT_NODE && (b.node.isBlock(j) || a(j).is(b.opts.htmlDoNotWrapTags.join(","))))
						h = null ;
						...
						j.nodeType == Node.TEXT_NODE && 0 == a(j).text().trim().length ? a(j).remove() : (null  == h && (h = a("<" + f + ">"),
						a(j).before(h)),
						j.nodeType == Node.TEXT_NODE && a(j).text().trim().length > 0 ? (h.append(a(j).clone()),
						a(j).remove()) : h.append(a(j)))
				}
		}
		function i(c, d, e, f) {
			return b.$wp ? ("undefined" == typeof c && (c = !1),
			"undefined" == typeof d && (d = !1),
			"undefined" == typeof e && (e = !1),
			"undefined" == typeof f && (f = !1),
			h(b.$el, c),
			f && b.$el.find(".fr-inner").each(function() {
				h(a(this), c)
			}),
		...
		function k() {
			for (var a = d(), c = 0; c < a.length; c++) {
				var e = a[c];
				"false" == e.getAttribute("contenteditable") || 0 != e.querySelectorAll(b.opts.htmlAllowedEmptyTags.join(",")).length || b.node.isVoid(e) || e.appendChild(b.doc.createElement("br"))
			}
		}
		function l() {
			return b.$el.find(f())
		...
		function m(a) {
			if ("undefined" == typeof a && (a = b.$el.get(0)),
			...
			for (var c = b.node.contents(a), d = c.length - 1; d >= 0; d--)
				if (c[d].nodeType == Node.TEXT_NODE) {
					...
					c[d].textContent = c[d].textContent.replace(/^[ ]{2,}/g, " "),
					c[d].textContent = c[d].textContent.replace(/[ ]{2,}$/g, " "),
					b.node.isBlock(a) && (c[d].previousSibling || (c[d].textContent = c[d].textContent.replace(/^ */, "")),
					c[d].nextSibling || (c[d].textContent = c[d].textContent.replace(/ *$/, "")),
					c[d].previousSibling && c[d].nextSibling && " " == c[d].textContent && (c[d].textContent = "\n"))
				} else
					m(c[d])
		}
		function n(a) {
		...
		function o(c) {
			if ("undefined" == typeof c && (c = b.$el.get(0)),
			!c.getAttribute || "false" != c.getAttribute("contenteditable"))
				if (c.nodeType == Node.ELEMENT_NODE && ["STYLE", "SCRIPT", "HEAD"].indexOf(c.tagName) < 0)
					for (var d = b.node.contents(c), e = d.length - 1; e >= 0; e--)
						(d[e].tagName != Node.ELEMENT_NODE || (d[e].className || "").indexOf("fr-marker") < 0) && o(d[e]);
					...
					if (n(f) && n(g) && 0 === c.textContent.trim().length)
						...
						for (var i = "", j = 0; j < h.length; j++)
							i += 32 != h.charCodeAt(j) || 0 !== j && 32 != i.charCodeAt(j - 1) ? h[j] : a.FE.UNICODE_NBSP;
						c.nextSibling || (i = i.replace(/ $/, a.FE.UNICODE_NBSP)),
						!c.previousSibling || b.node.isVoid(c.previousSibling) || b.node.isBlock(c.previousSibling) || (i = i.replace(/^\u00A0([^ $])/, " $1")),
						i = i.replace(/([^ \u00A0])\u00A0([^ \u00A0])/g, "$1 $2"),
		...
		function p(c) {
			...
			c.nodeType == Node.ELEMENT_NODE && ["STYLE", "SCRIPT", "HEAD"].indexOf(c.tagName) < 0) {
				...
				var g = c.previousSibling
				  , h = c.nextSibling;
				if (n(g) && n(h) && 0 === c.textContent.trim().length)
					return !0;
				var i = c.textContent;
				i = i.replace(new RegExp(a.FE.UNICODE_NBSP,"g"), " ");
				for (var j = "", k = 0; k < i.length; k++)
					j += 32 != i.charCodeAt(k) || 0 !== k && 32 != j.charCodeAt(k - 1) ? i[k] : a.FE.UNICODE_NBSP;
				if (c.nextSibling || (j = j.replace(/ $/, a.FE.UNICODE_NBSP)),
				c.previousSibling && !b.node.isVoid(c.previousSibling) && (j = j.replace(/^\u00A0([^ $])/, " $1")),
				j = j.replace(/([^ \u00A0])\u00A0([^ \u00A0])/g, "$1 $2"),
				c.textContent != j)
					return !0
			}
		...
		function u() {
			b.core.isEmpty() && (null  != c() ? 0 === b.$el.get(0).querySelectorAll(f()).length && 0 === b.$el.get(0).querySelectorAll(b.opts.htmlDoNotWrapTags.join(",")).length && (b.core.hasFocus() ? (b.$el.html("<" + c() + ">" + a.FE.MARKERS + "<br/></" + c() + ">"),
			b.selection.restore()) : b.$el.html("<" + c() + "><br/></" + c() + ">")) : 0 === b.$el.get(0).querySelectorAll("*:not(.fr-marker):not(br)").length && (b.core.hasFocus() ? (b.$el.html(a.FE.MARKERS + "<br/>"),
		...
		function y(a) {
			...
			if (b.opts.fullPage) {
				...
				b.$html.attr(i),
				b.iframe_document.doctype.parentNode.replaceChild(r(h, b.iframe_document), b.iframe_document.doctype)
			} else
				b.$el.html(c);
		...
		function z(a, c) {
			...
			if (!b.opts.useClasses && !c) {
				for (e = 0; e < b.doc.styleSheets.length; e++) {
					...
					try {
						...
						b.doc.styleSheets[e].ownerNode && "STYLE" == b.doc.styleSheets[e].ownerNode.nodeType && (j = 1)
					} catch (k) {}
					if (i)
						for (var l = 0, m = i.length; m > l; l++) {
							...
							if (i[l].selectorText && 0 === i[l].selectorText.indexOf(n) && i[l].style.cssText.length > 0)
								for (var o = i[l].selectorText.replace(n, "").replace(/::/g, ":"), p = b.$el.get(0).querySelectorAll(o), q = 0; q < p.length; q++) {
									!p[q].getAttribute("fr-original-style") && p[q].getAttribute("style") ? (p[q].setAttribute("fr-original-style", p[q].getAttribute("style")),
									g.push(p[q])) : p[q].getAttribute("fr-original-style") || g.push(p[q]),
									h[p[q]] || (h[p[q]] = {});
									for (var r = 1e3 * j + f(i[l].selectorText), t = i[l].style.cssText.split(";"), u = 0; u < t.length; u++) {
										var v = t[u].trim().split(":")[0];
										h[p[q]][v] || (h[p[q]][v] = 0,
										(p[q].getAttribute("fr-original-style") || "").indexOf(v + ":") >= 0 && (h[p[q]][v] = 1e4)),
										r >= h[p[q]][v] && (h[p[q]][v] = r,
										t[u].trim().length && (p[q].style[v.trim()] = t[u].trim().split(":")[1]))
									}
								}
						}
				}
				for (e = 0; e < g.length; e++)
					g[e].getAttribute("class") && (g[e].setAttribute("fr-original-class", g[e].getAttribute("class")),
			...
			b.opts.fullPage ? (d = s(b.iframe_document),
			d += "<html" + b.node.attributes(b.$html.get(0)) + ">" + b.$html.html() + "</html>") : d = b.$el.html()),
			!b.opts.useClasses && !c)
				for (e = 0; e < g.length; e++)
		...
		function A() {
			var c = function(c, d) {
			  ...
			  , d = function() {
				...
				null  != d && (a.inArray(b.$el.get(0), a(d).parents()) >= 0 || d == b.$el.get(0)) ? d : null 
			}
			  , e = "";
		...
		function E(c, d, e) {
			...
			f = f.replace(/\r|\n/g, " "),
			c.indexOf('class="fr-marker"') < 0 && (f = C(f)),
			b.core.isEmpty())
				b.$el.html(f);
			else {
		...
		function F(c) {
			...
			do {
				f = !1,
				e = b.$el.get(0).querySelectorAll("*:not(.fr-marker)");
				for (var g = 0; g < e.length; g++) {
					var h = e[g];
					if (d != h) {
						var i = h.textContent;
						0 === h.children.length && 1 === i.length && 8203 == i.charCodeAt(0) && (a(h).remove(),
						f = !0)
					}
				}
			} while (f)
		...
		function G() {
			var a = function() {
				F(),
				b.placeholder && b.placeholder.refresh()
			}
			;
	...
	a.FE.MODULES.size = function(a) {
		...
		function c() {
			...
			a.$el.css("minHeight", a.opts.height - a.helpers.getPX(a.$el.css("padding-top")) - a.helpers.getPX(a.$el.css("padding-bottom"))),
			a.$wp.css("overflow", "auto")) : (a.$wp.css("height", ""),
		...
		function d() {
			return a.$wp ? (c(),
			void (a.opts.iframe && (a.events.on("keyup", b),
			a.events.on("commands.after", b),
			a.events.on("html.set", b),
			a.events.on("init", b),
			a.events.on("initialized", b)))) : !1
		}
		return {
			_init: d,
			syncIframe: b,
	...
	a.FE.MODULES.placeholder = function(b) {
		function c() {
			...
			d.length && d[0].nodeType == Node.ELEMENT_NODE ? (b.opts.toolbarInline || (c = b.helpers.getPX(a(d[0]).css("margin-top"))),
			b.$placeholder.css("font-size", a(d[0]).css("font-size")),
			b.$placeholder.css("line-height", a(d[0]).css("line-height"))) : (b.$placeholder.css("font-size", b.$el.css("font-size")),
			b.$placeholder.css("line-height", b.$el.css("line-height"))),
			b.$wp.addClass("show-placeholder"),
			b.$placeholder.css("margin-top", Math.max(b.helpers.getPX(b.$el.css("padding-top")), c)).text(b.language.translate(b.opts.placeholderText || b.$oel.attr("placeholder") || ""))
		}
		function d() {
	...
	a.FE.MODULES.core = function(b) {
		...
		function h(a, c) {
			...
			d.open(c, a, !0),
			b.opts.requestWithCORS && (d.withCredentials = !0);
			for (var e in b.opts.requestHeaders)
				b.opts.requestHeaders.hasOwnProperty(e) && d.setRequestHeader(e, b.opts.requestHeaders[e]);
			return d
		}
		function i() {
			"TEXTAREA" == b.$oel.get(0).tagName && b.$oel.val(b.html.get()),
			b.$wp && ("TEXTAREA" == b.$oel.get(0).tagName ? (b.$box.replaceWith(b.$oel),
			b.$oel.show()) : (b.$wp.replaceWith(b.html.get()),
	...
	a.FE.MODULES.commands = function(b) {
		function c(c, d) {
			if (b.events.trigger("commands.before", a.merge([c], d || [])) !== !1) {
				...
				b.core.hasFocus() || !f || b.popups.areVisible() || b.events.focus(!0),
		...
		function d(c, d) {
			if (b.selection.isCollapsed() && b.doc.queryCommandState(c) === !1) {
				...
				e.replaceWith("<" + d + ">" + a.FE.INVISIBLE_SPACE + a.FE.MARKERS + "</" + d + ">"),
				b.selection.restore()
			} else {
				var f = b.selection.element();
				if (b.selection.isCollapsed() && b.doc.queryCommandState(c) === !0 && f.tagName == d.toUpperCase() && 0 === (f.textContent || "").replace(/\u200B/g, "").length)
					a(f).replaceWith(a.FE.MARKERS),
					b.selection.restore();
				else {
					var g = b.$el.find("span")
					  , h = !1;
					b.doc.queryCommandState(c) !== !1 || b.browser.chrome || (b.selection.save(),
					h = !0),
					b.browser.mozilla && b.$el.find(".fr-marker").css("display", "inline-block"),
					b.doc.execCommand(c, !1, !1),
					h && b.selection.restore();
					var i = b.$el.find("span[style]").not(g).filter(function() {
						return a(this).attr("style").indexOf("font-weight: normal") >= 0
					});
					i.length && (b.selection.save(),
					i.each(function() {
		...
		function f() {
			...
			b.$el.find("[style]").each(function() {
				var b = a(this);
				c(b) && (b.attr("data-font-size", b.css("font-size")),
				b.css("font-size", ""))
			})
		}
		function g() {
			b.$el.find("[data-font-size]").each(function() {
				var b = a(this);
				b.css("font-size", b.attr("data-font-size")),
				b.removeAttr("data-font-size")
			})
		}
		function h() {
		...
		function i(c, d) {
			if (b.selection.isCollapsed()) {
				b.markers.insert();
				var e = b.$el.find(".fr-marker");
				e.replaceWith('<span style="' + c + ": " + d + ';">' + a.FE.INVISIBLE_SPACE + a.FE.MARKERS + "</span>"),
				b.selection.restore()
			} else {
				f(),
				b.doc.execCommand("fontSize", !1, 4),
				b.selection.save(),
				g();
				...
				; b.$el.find("font").length > 0; ) {
					...
					if (o.length) {
						...
						do
							t = t.parentNode,
							p += b.node.closeTagString(t),
							q = b.node.openTagString(t) + q,
							o.get(0) != t && (r += b.node.closeTagString(t),
							s = b.node.openTagString(t) + s);
						while (o.get(0) != t);var u = p + '<span class="fr-just" style="' + c + ": " + d + ';">' + s + m.html() + r + "</span>" + q;
		...
		var k = {
			...
			insertHR: function() {
				b.selection.remove(),
				b.html.insert('<hr id="fr-just">');
				var a = b.$el.find("hr#fr-just");
				a.removeAttr("id"),
				b.selection.setAfter(a.get(0)) || b.selection.setBefore(a.get(0)),
				b.selection.restore()
			},
			clearFormatting: function() {
				if (b.browser.msie || b.browser.edge) {
					var c = function(c) {
						b.commands.applyProperty(c, "#123456"),
						b.selection.save(),
						b.$el.find("span:not(.fr-marker)").each(function(d, e) {
							var f = a(e)
							  , g = f.css(c);
				...
				b.doc.execCommand("removeFormat", !1, !1),
				b.doc.execCommand("unlink", !1, !1)
			},
			selectAll: function() {
	...
	a.FE.MODULES.cursorLists = function(b) {
		...
		function e(e) {
			...
			if (b.node.isEmpty(g, !0) && h) {
				...
				i && h || !b.node.isEmpty(g, !0) ? (a(g).before("<li><br></li>"),
				a(e).remove()) : i ? (f = d(g),
				f.parentNode && "LI" == f.parentNode.tagName ? a(f.parentNode).after("<li>" + a.FE.MARKERS + "<br></li>") : j ? a(f).after("<" + j + ">" + a.FE.MARKERS + "<br></" + j + ">") : a(f).after(a.FE.MARKERS + "<br>"),
				a(g).remove()) : (f = d(g),
				f.parentNode && "LI" == f.parentNode.tagName ? a(f.parentNode).before("<li>" + a.FE.MARKERS + "<br></li>") : j ? a(f).before("<" + j + ">" + a.FE.MARKERS + "<br></" + j + ">") : a(f).before(a.FE.MARKERS + "<br>"),
				a(g).remove())
		}
		function f(d) {
	...
	a.FE.MODULES.cursor = function(b) {
				...
				if (b.node.isBlock(e) && b.node.isEditable(e) && h && a.FE.NO_DELETE_TAGS.indexOf(h.tagName) < 0 && b.node.isEditable(h))
					if (b.node.isBlock(h))
						if (b.node.isEmpty(h) && !b.node.isList(h))
							...
							a(h).append(b.node.isEmpty(e) ? a.FE.MARKERS : e.innerHTML),
							a(e).remove()
						}
					else
						a(c).replaceWith(a.FE.MARKERS),
		...
		function i(c) {
			...
			if (!b.node.isBlock(d) && b.node.isEditable(d)) {
				for (e = b.node.contents(d); d.nodeType != Node.TEXT_NODE && e.length && b.node.isEditable(d); )
					d = e[e.length - 1],
					e = b.node.contents(d);
				if (d.nodeType == Node.TEXT_NODE) {
					...
					d.textContent.length && 55357 == d.textContent.charCodeAt(d.textContent.length - 1) && (d.textContent = d.textContent.substr(0, d.textContent.length - 1)),
					0 == d.textContent.length && d.parentNode.removeChild(d)
				} else
					b.events.trigger("node.remove", [a(d)]) !== !1 && (a(d).after(a.FE.MARKERS),
					a(d).remove())
		...
		function j() {
			...
			if (k) {
				var l = k.textContent;
				l && l.length && 8203 == l.charCodeAt(l.length - 1) && (1 == l.length ? a(k).remove() : (k.textContent = k.textContent.substr(0, l.length - 1),
				k.textContent.length && 55357 == k.textContent.charCodeAt(k.textContent.length - 1) && (k.textContent = k.textContent.substr(0, k.textContent.length - 1))))
			}
			return c(j) ? f = i(j) : d(j) ? g(j) && e(j, a(j).parents("li:first").get(0)) ? b.cursorLists._backspace(j) : h(j) : f = i(j),
			a(j).remove(),
		...
		function l(d) {
			...
			if (!b.node.isBlock(e) && b.node.isEditable(e)) {
				...
				e.textContent.length && 55357 == e.textContent.charCodeAt(0) ? e.textContent = e.textContent.substring(2, e.textContent.length) : e.textContent = e.textContent.substring(1, e.textContent.length)) : b.events.trigger("node.remove", [a(e)]) !== !1 && (a(e).before(a.FE.MARKERS),
				a(e).remove()),
				a(d).remove()
		...
		function o(c, d, e) {
			...
			if (h && "BLOCKQUOTE" == h.tagName)
				...
				!1);
			if (null  == h)
				a(c).replaceWith("<br/>" + a.FE.MARKERS + "<br/>");
			else {
				var i = c
				  , j = "";
		...
		function q(c, d, g) {
			var h = b.node.deepestParent(c, [], !g);
			if (null  == h)
				b.html.defaultTag() && c.parentNode === b.$el.get(0) ? a(c).replaceWith("<" + b.html.defaultTag() + ">" + a.FE.MARKERS + "<br></" + b.html.defaultTag() + ">") : ((!c.nextSibling || b.node.isBlock(c.nextSibling)) && a(c).after("<br>"),
				a(c).replaceWith("<br>" + a.FE.MARKERS));
			else {
				var i = c
				  , j = "";
				...
				do {
					var m = i;
					if (i = i.parentNode,
					"BLOCKQUOTE" == h.tagName && b.node.isEmpty(m) && !a(m).hasClass("fr-marker") && a(m).find(c).length > 0 && a(m).after(c),
		...
		function r(e) {
			...
			c(f) ? !g(f) || e || h ? o(f, e, h) : b.cursorLists._endEnter(f) : d(f) ? !g(f) || e || h ? p(f, e, h) : b.cursorLists._startEnter(f) : !g(f) || e || h ? q(f, e, h) : b.cursorLists._middleEnter(f),
			n(),
			b.html.fillEmptyBlocks(),
			b.html.cleanEmptyTags(),
			b.clean.lists(),
	...
	a.FE.MODULES.data = function(a) {
		...
		function f(a) {
			return a && "none" == a.css("display") ? (a.remove(),
			!0) : !1
		}
		function g() {
			return f(j) || f(k)
		...
		function h() {
			...
			k = j.find("> a"),
			void ("rtl" == a.opts.direction && j.css("left", "auto").css("right", 0))) : !1
		}
		function i() {
			var c = a.opts.key || [""];
			"string" == typeof c && (c = [c]),
	...
	a.FE.MODULES.keys = function(b) {
		...
		function f(a) {
			b.selection.isCollapsed() ? b.cursor.backspace() || (a.preventDefault(),
			a.stopPropagation(),
			x = !1) : (a.preventDefault(),
			a.stopPropagation(),
			b.selection.remove(),
			b.html.fillEmptyBlocks(),
			x = !1),
		...
		function h(c) {
			if (b.browser.mozilla) {
				...
				var d = b.$el.find(".fr-marker").get(0)
				  , e = d.previousSibling
				  , f = d.nextSibling;
				!f && d.parentNode && "A" == d.parentNode.tagName ? (a(d).parent().after("&nbsp;" + a.FE.MARKERS),
				a(d).remove()) : (e && e.nodeType == Node.TEXT_NODE && 1 == e.textContent.length && 160 == e.textContent.charCodeAt(0) ? a(e).after(" ") : a(d).before("&nbsp;"),
				a(d).replaceWith(a.FE.MARKERS)),
				b.selection.restore()
			}
		}
		...
		function n(c) {
			...
			i == a.FE.KEYCODE.ENTER ? c.shiftKey ? e(c) : d(c) : i != a.FE.KEYCODE.BACKSPACE || r(c) ? i != a.FE.KEYCODE.DELETE || r(c) ? i == a.FE.KEYCODE.SPACE ? h(c) : i == a.FE.KEYCODE.TAB ? k(c) : r(c) || !s(c.which) || b.selection.isCollapsed() || b.selection.remove() : g(c) : f(c),
			b.events.enableBlur()
		}
		function o(c) {
			for (var d = 0; d < c.length; d++)
				c[d].nodeType == Node.TEXT_NODE && /\u200B/gi.test(c[d].textContent) ? (c[d].textContent = c[d].textContent.replace(/\u200B/gi, ""),
				0 === c[d].textContent.length && a(c[d]).remove()) : c[d].nodeType == Node.ELEMENT_NODE && "IFRAME" != c[d].nodeType && o(b.node.contents(c[d]))
		}
		function p() {
			if (!b.$wp)
				return !0;
			...
			c > b.$wp.offset().top - a(b.o_win).scrollTop() + b.$wp.height() - 20 && b.$wp.scrollTop(c + b.$wp.scrollTop() - (b.$wp.height() + b.$wp.offset().top) + a(b.o_win).scrollTop() + 20)) : (c = b.position.getBoundingRect().top,
			b.helpers.isIOS() && (c -= a(b.o_win).scrollTop()),
			b.opts.iframe && (c += b.$iframe.offset().top),
			c > b.o_win.innerHeight - 20 && a(b.o_win).scrollTop(c + a(b.o_win).scrollTop() - b.o_win.innerHeight + 20),
			c = b.position.getBoundingRect().top,
			b.helpers.isIOS() && (c -= a(b.o_win).scrollTop()),
			b.opts.iframe && (c += b.$iframe.offset().top),
			c < b.$tb.height() + 20 && a(b.o_win).scrollTop(c + a(b.o_win).scrollTop() - b.$tb.height() - 20))
		}
		function q(c) {
			if (A)
				return !1;
			...
			var m = function(b) {
				if (!b)
					return !1;
				var c = a(b).html();
				return c = c.replace(/<span[^>]*? class\s*=\s*["']?fr-marker["']?[^>]+>\u200b<\/span>/gi, ""),
				c && /\u200B/.test(c) && c.replace(/\u200B/gi, "").length > 0 ? !0 : !1
			}
			  , n = function(a) {
				var c = /[\u3041-\u3096\u30A0-\u30FF\u4E00-\u9FFF\u3130-\u318F\uAC00-\uD7AF]/gi;
			...
			m(q) && 0 === a(q).find("li").length && !a(q).hasClass("fr-marker") && "IFRAME" != q.tagName && n(q) && (b.selection.save(),
			o(b.node.contents(q)),
			b.selection.restore()),
			!b.browser.mozilla && b.html.doNormalize() && (b.selection.save(),
			b.html.normalizeSpaces(),
			b.selection.restore())
		}
		function r(a) {
		...
		function t(a) {
			var c = a.which;
			return r(a) || c >= 37 && 40 >= c ? !0 : (y || (z = b.snapshot.get()),
			clearTimeout(y),
		...
		function u(a) {
			return r(a) ? !0 : void (z && y && (b.undo.saveStep(z),
			z = null ))
		}
		function v() {
			y && (clearTimeout(y),
	...
	a.FE.MODULES.paste = function(b) {
		function c(c) {
			...
			a.FE.copied_text = a("<div>").html(a.FE.copied_html).text(),
			"cut" == c.type && (b.undo.saveStep(),
			setTimeout(function() {
				b.html.wrap(),
				b.events.focus(),
				b.undo.saveStep()
		...
		function d(a) {
			...
			a && a.clipboardData && a.clipboardData.getData) {
				...
				if (b.helpers.isArray(d))
					...
					c = d;
				if (m = "",
				/text\/html/.test(c) ? m = a.clipboardData.getData("text/html") : /text\/rtf/.test(c) && b.browser.safari ? m = a.clipboardData.getData("text/rtf") : /text\/plain/.test(c) && !this.browser.mozilla && (m = b.html.escapeEntities(a.clipboardData.getData("text/plain")).replace(/\n/g, "<br>")),
				"" !== m)
					return h(),
		...
		function e() {
			...
			b.events.disableBlur(),
			m = null ,
			n ? n.html("") : (n = a('<div contenteditable="true" style="position: fixed; top: 0; left: -9999px; height: 100%; width: 0; z-index: 9999; line-height: 140%;" tabindex="-1"></div>'),
			b.$box.after(n)),
			n.focus(),
			b.win.setTimeout(h, 1)
		}
		function f(c) {
			c = c.replace(/<p(.*?)class="?'?MsoListParagraph"?'? ([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<ul><li>$3</li></ul>"),
			c = c.replace(/<p(.*?)class="?'?NumberedText"?'? ([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<ol><li>$3</li></ol>"),
			c = c.replace(/<p(.*?)class="?'?MsoListParagraphCxSpFirst"?'?([\s\S]*?)(level\d)?([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<ul><li$3>$5</li>"),
			c = c.replace(/<p(.*?)class="?'?NumberedTextCxSpFirst"?'?([\s\S]*?)(level\d)?([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<ol><li$3>$5</li>"),
			c = c.replace(/<p(.*?)class="?'?MsoListParagraphCxSpMiddle"?'?([\s\S]*?)(level\d)?([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<li$3>$5</li>"),
			c = c.replace(/<p(.*?)class="?'?NumberedTextCxSpMiddle"?'?([\s\S]*?)(level\d)?([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<li$3>$5</li>"),
			c = c.replace(/<p(.*?)class="?'?MsoListBullet"?'?([\s\S]*?)(level\d)?([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<li$3>$5</li>"),
			c = c.replace(/<p(.*?)class="?'?MsoListParagraphCxSpLast"?'?([\s\S]*?)(level\d)?([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<li$3>$5</li></ul>"),
			c = c.replace(/<p(.*?)class="?'?NumberedTextCxSpLast"?'?([\s\S]*?)(level\d)?([\s\S]*?)>([\s\S]*?)<\/p>/gi, "<li$3>$5</li></ol>"),
			c = c.replace(/<span([^<]*?)style="?'?mso-list:Ignore"?'?([\s\S]*?)>([\s\S]*?)<span/gi, "<span><span"),
			c = c.replace(/<!--\[if \!supportLists\]-->([\s\S]*?)<!--\[endif\]-->/gi, ""),
			c = c.replace(/<!\[if \!supportLists\]>([\s\S]*?)<!\[endif\]>/gi, ""),
			c = c.replace(/(\n|\r| class=(")?Mso[a-zA-Z0-9]+(")?)/gi, " "),
			c = c.replace(/<!--[\s\S]*?-->/gi, ""),
			c = c.replace(/<(\/)*(meta|link|span|\\?xml:|st1:|o:|font)(.*?)>/gi, "");
			...
			do
				g = c,
				c = c.replace(/<[^\/>][^>]*><\/[^>]+>/gi, "");
			while (c != g);c = c.replace(/<lilevel([^1])([^>]*)>/gi, '<li data-indent="true"$2>'),
		...
		function h() {
			...
			if ("string" == typeof d && (m = d),
			m.indexOf("<body") >= 0 && (m = m.replace(/[.\s\S\w\W<>]*<body[^>]*>([.\s\S\w\W<>]*)<\/body>[.\s\S\w\W<>]*/g, "$1")),
			m.indexOf('id="docs-internal-guid') >= 0 && (m = m.replace(/^.* id="docs-internal-guid[^>]*>(.*)<\/b>.*$/, "$1")),
			m.match(/(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/gi) ? (m = m.replace(/^\n*/g, "").replace(/^ /g, ""),
			0 === m.indexOf("<colgroup>") && (m = "<table>" + m + "</table>"),
			m = f(m),
			m = j(m)) : (b.opts.htmlAllowComments = !1,
			m = b.clean.html(m, b.opts.pasteDeniedTags, b.opts.pasteDeniedAttrs),
			b.opts.htmlAllowComments = !0,
			m = j(m),
			m = m.replace(/\r|\n|\t/g, ""),
			a.FE.copied_text && a("<div>").html(m).text().replace(/(\u00A0)/gi, " ").replace(/\r|\n/gi, "") == a.FE.copied_text.replace(/(\u00A0)/gi, " ").replace(/\r|\n/gi, "") && (m = a.FE.copied_html),
			...
			"" !== m) {
				var e = a("<div>").html(m);
				b.html.normalizeSpaces(e.get(0)),
		...
		function j(b) {
			...
			for (var f = d.find("> div:not([style]), td > div, th > div, li > div"); f.length; ) {
				...
				g.replaceWith(g.html() + "<br>"),
				f = d.find("> div:not([style]), td > div, th > div, li > div")
			}
	...
	a.FE.MODULES.tooltip = function(b) {
		...
		function e(e, f, g) {
			...
			b.events.$on(e, "mouseleave " + b._mousedown + " " + b._mouseup, f, function(a) {
				c()
			}, !0))
		}
		function f() {
			b.helpers.isMobile() || (b.shared.$tooltip ? b.$tooltip = b.shared.$tooltip : (b.shared.$tooltip = a('<div class="fr-tooltip"></div>'),
	...
	a.FE.MODULES.button = function(b) {
		function c(c) {
			var d = a(c.currentTarget)
			  ...
			  , g = (b.helpers.isMobile(),
			a(".fr-dropdown.fr-active").not(d))
			  , h = d.parents(".fr-toolbar, .fr-popup").data("instance") || b;
			if (h.helpers.isIOS() && 0 == h.$el.get(0).querySelectorAll(".fr-marker").length && (h.selection.save(),
			h.selection.clear(),
			h.selection.restore()),
			!f) {
				var i = d.data("cmd");
				e.find(".fr-command").removeClass("fr-active"),
				a.FE.COMMANDS[i] && a.FE.COMMANDS[i].refreshOnShow && a.FE.COMMANDS[i].refreshOnShow.apply(h, [d, e]),
				e.css("left", d.offset().left - d.parent().offset().left - ("rtl" == b.opts.direction ? e.width() - d.outerWidth() : 0)),
				b.opts.toolbarBottom ? e.css("bottom", b.$tb.height() - d.position().top) : e.css("top", d.position().top + d.outerHeight())
			}
			d.addClass("fr-blink").toggleClass("fr-active"),
			setTimeout(function() {
				d.removeClass("fr-blink")
			}, 300),
			e.offset().left + e.outerWidth() > a(b.opts.scrollableContainer).offset().left + a(b.opts.scrollableContainer).outerWidth() && e.css("margin-left", -(e.offset().left + e.outerWidth() - a(b.opts.scrollableContainer).offset().left - a(b.opts.scrollableContainer).outerWidth())),
			g.removeClass("fr-active"),
			g.parent(".fr-toolbar:not(.fr-inline)").css("zIndex", ""),
			0 != d.parents(".fr-popup").length || b.opts.toolbarInline || (d.hasClass("fr-active") ? b.$tb.css("zIndex", (b.opts.zIndex || 1) + 4) : b.$tb.css("zIndex", ""))
		}
		function d(b) {
			b.addClass("fr-blink"),
			setTimeout(function() {
			...
			for (var c = b.data("cmd"), d = []; "undefined" != typeof b.data("param" + (d.length + 1)); )
				d.push(b.data("param" + (d.length + 1)));
			var e = a(".fr-dropdown.fr-active");
			e.length && (e.removeClass("fr-active"),
			e.parent(".fr-toolbar:not(.fr-inline)").css("zIndex", "")),
			b.parents(".fr-popup, .fr-toolbar").data("instance").commands.exec(c, d)
		}
		function e(b) {
		...
		function f(b) {
			...
			if (0 != d.parents(".fr-popup").length || d.data("popup") || f.popups.hideAll(),
			f.popups.areVisible() && !f.popups.areVisible(f)) {
				for (var g = 0; g < a.FE.INSTANCES.length; g++)
					a.FE.INSTANCES[g] != f && a.FE.INSTANCES[g].popups && a.FE.INSTANCES[g].popups.areVisible() && a.FE.INSTANCES[g].$el.find(".fr-marker").remove();
				f.popups.hideAll()
			}
			d.hasClass("fr-dropdown") ? c(b) : (e(b),
			a.FE.COMMANDS[d.data("cmd")] && 0 != a.FE.COMMANDS[d.data("cmd")].refreshAfterCallback && f.button.bulkRefresh())
		}
		function g(a) {
		...
		function i(a) {
			...
			b.opts.toolbarInline ? !1 : void 0
		}
		function j(c, d) {
			b.events.bindClick(c, ".fr-command:not(.fr-disabled)", f),
			b.events.$on(c, b._mousedown + " " + b._mouseup + " " + b._move, ".fr-dropdown-menu", h, !0),
			b.events.$on(c, b._mousedown + " " + b._mouseup + " " + b._move, ".fr-dropdown-menu .fr-dropdown-wrapper", i, !0);
			...
			a.merge(q, c.find(".fr-btn").toArray()),
		...
		function k(a, c) {
			...
			if (c.html)
				...
				for (var f in e)
					e.hasOwnProperty(f) && (d += '<li><a class="fr-command" data-cmd="' + a + '" data-param1="' + f + '" title="' + e[f] + '">' + b.language.translate(e[f]) + "</a></li>");
				d += "</ul>"
			}
			return d
		}
			...
			for (var e = "", f = 0; f < c.length; f++) {
				...
				if (!(h && "undefined" != typeof h.plugin && b.opts.pluginsEnabled.indexOf(h.plugin) < 0))
					if (h) {
						...
						"|" == g ? e += '<div class="fr-separator fr-vs"></div>' : "-" == g && (e += '<div class="fr-separator fr-hs"></div>')
			}
			return e
		}
		function n(c) {
			var d, e = c.parents(".fr-popup, .fr-toolbar").data("instance") || b, f = c.data("cmd");
			c.hasClass("fr-dropdown") ? d = c.next() : c.removeClass("fr-active"),
		...
		function p() {
			b.opts.toolbarInline ? b.events.on("toolbar.show", o) : (b.events.on("mouseup", o),
			b.events.on("keyup", o),
			b.events.on("blur", o),
			b.events.on("focus", o),
			b.events.on("contentChanged", o))
		}
		var q = [];
		return (b.opts.toolbarInline || b.opts.toolbarContainer) && (b.shared.buttons || (b.shared.buttons = []),
		q = b.shared.buttons),
		{
			_init: p,
			buildList: m,
	...
	a.FE.MODULES.position = function(b) {
		function c() {
			...
			if (d && d.collapsed && b.selection.inEditor()) {
				...
				c.width = 0,
				c.height = h,
				c.top = g.top - (b.helpers.isIOS() ? 0 : a(b.o_win).scrollTop()),
				c.right = 1,
				c.bottom = 1,
				c.ok = !0,
		...
		function d(c, d, e) {
			...
			if (!b.helpers.isMobile() && b.$tb && c.parent().get(0) != b.$tb.get(0)) {
				var g = (c.parent().height() - 20 - (b.opts.toolbarBottom ? b.$tb.outerHeight() : 0),
				c.parent().offset().top)
				  , h = d - f - (e || 0);
				c.parent().get(0) == a(b.opts.scrollableContainer).get(0) && (g -= c.parent().position().top),
				g + d + f > a(b.o_doc).outerHeight() && c.parent().offset().top + h > 0 ? (d = h,
				c.addClass("fr-above")) : c.removeClass("fr-above")
			}
			return d
		}
		function e(c, d) {
			var e = c.outerWidth();
			return c.parent().offset().left + d + e > a(b.opts.scrollableContainer).width() - 10 && (d = a(b.opts.scrollableContainer).width() - e - 10 - c.parent().offset().left + a(b.opts.scrollableContainer).offset().left),
			c.parent().offset().left + d < a(b.opts.scrollableContainer).offset().left && (d = 10 - c.parent().offset().left + a(b.opts.scrollableContainer).offset().left),
			d
		}
		function f(d) {
			var e = c();
			...
			b.opts.iframe || (f += a(b.o_win).scrollTop()),
			g(h, f, d, e.height)
		}
		function g(a, c, f, g) {
			var h = f.data("container");
			h && "BODY" != h.get(0).tagName && (a && (a -= h.offset().left),
			c && (c -= h.offset().top - h.scrollTop())),
		...
		function h(c) {
			...
			if (b.core.hasFocus() || b.$tb.find("input:visible:focus").length > 0) {
				...
				d.data("sticky-timeout", setTimeout(function() {
					...
					e > 0 && "BODY" == b.$tb.parent().get(0).tagName && (e += b.$tb.parent().position().top),
					e != f && (d.css("top", Math.max(e, 0)),
					d.data("sticky-top", e),
		...
		function i(c) {
			if (c.offsetWidth) {
				...
				"body" !== b.opts.scrollableContainer && (j = a(b.opts.scrollableContainer).offset().top,
				...
				!h) {
					...
					f.data("top", f.css("top")),
					f.data("bottom", f.css("bottom"))
				}
				var r = function() {
				...
				f.css("bottom") && ("auto" != f.data("bottom") ? f.css("bottom", b.helpers.getPX(f.data("bottom")) + k) : f.css("bottom", "auto")))) : f.hasClass("fr-sticky-off") || (f.width(""),
		...
		function j() {
			...
			-1 !== c.position.indexOf("sticky") && !b.helpers.isIOS() && !b.helpers.isAndroid()
		}
		function k() {
			if (!j())
	...
	a.FE.MODULES.toolbar = function(b) {
		...
		function k() {
			...
			b.browser.mozilla && setInterval(h, 200)) : b.events.on("window.keyup", h),
			b.events.on("keydown", function(b) {
				b && b.which == a.FE.KEYCODE.ESC && i()
			}),
			b.events.$on(b.$wp, "scroll.toolbar", h),
			b.events.on("commands.after", h),
			b.helpers.isMobile() && (b.events.$on(b.$doc, "selectionchange", h),
			b.events.$on(b.$doc, "orientationchange", h))
		}
		function l() {
			b.opts.toolbarInline ? (a(b.opts.scrollableContainer).append(b.$tb),
		...
		function o() {
			...
			b.opts.toolbarContainer ? (b.opts.toolbarInline && (k(),
			i()),
			b.opts.toolbarBottom ? b.$tb.addClass("fr-bottom") : b.$tb.addClass("fr-top")) : l(),
			...
			b.events.$on(b.$tb, b._mousedown + " " + b._mouseup, function(a) {
				...
				return b && "INPUT" != b.tagName ? (a.stopPropagation(),
				a.preventDefault(),
				!1) : void 0
			}, !0)
		}
		function p() {
			return b.$wp ? (b.opts.toolbarContainer ? (b.shared.$tb ? (b.$tb = b.shared.$tb,
			b.opts.toolbarInline && k()) : (b.shared.$tb = a('<div class="fr-toolbar"></div>'),
			b.$tb = b.shared.$tb,
			...
			k()) : (b.shared.$tb = a('<div class="fr-toolbar"></div>'),
			b.$tb = b.shared.$tb,
			o())) : (b.$box.addClass("fr-basic"),
			b.$tb = a('<div class="fr-toolbar"></div>'),
			o(),
			b.$tb.data("instance", b)),
			b.events.on("destroy", n, !0),
			void b.events.on(b.opts.toolbarInline ? "shared.destroy" : "destroy", m, !0)) : !1
		}
		function q() {
			!v && b.$tb && (b.$tb.find("> .fr-command").addClass("fr-disabled fr-no-refresh"),
			v = !0)
		}
		function r() {
			v && b.$tb && (b.$tb.find("> .fr-command").removeClass("fr-disabled fr-no-refresh"),
			v = !1),
			b.button.bulkRefresh()
		}
		var s, t, u = [];
		...
		function c(c) {
			...
			if (b.keys.ctrlKey(c) && (c.shiftKey && a.FE.SHORTCUTS_MAP[-d] || !c.shiftKey && a.FE.SHORTCUTS_MAP[d])) {
				...
				if (e && b.opts.shortcutsEnabled.indexOf(e) >= 0) {
					...
					if (e && !g ? f = b.$tb.find('.fr-command[data-cmd="' + e + '"]') : e && g && (f = b.$tb.find('.fr-command[data-cmd="' + e + '"][data-param1="' + g + '"]')),
					f.length)
						return c.preventDefault(),
						c.stopPropagation(),
		...
		function f() {
			...
			a.$wp && a.selection.inEditor() && a.core.hasFocus())
				for (var c = a.selection.ranges(), d = 0; d < c.length; d++)
					b.ranges.push(e(c[d]));
			return a.events.trigger("snapshot.after"),
			b
		}
		function g(b) {
	...
	a.FE.MODULES.undo = function(a) {
		...
		function e(b) {
			return !a.undo_stack || a.undoing || a.$el.get(0).querySelectorAll(".fr-marker").length ? !1 : (f(),
			void ("undefined" == typeof b ? (b = a.snapshot.get(),
			a.undo_stack[a.undo_index - 1] && a.snapshot.equal(a.undo_stack[a.undo_index - 1], b) || (a.undo_stack.push(b),
			a.undo_index++,
			b.html != k && (a.events.trigger("contentChanged"),
			k = b.html))) : a.undo_index > 0 ? a.undo_stack[a.undo_index - 1] = b : (a.undo_stack.push(b),
			a.undo_index++)))
		}
		function f() {
	...
	a.FE.MODULES.popups = function(b) {
		function c(c, d) {
			...
			d.is(x[c].data("container")) || (x[c].data("container", d),
			d.append(x[c]))
		}
		function d(d, e, h, i) {
			if (g() && b.$el.find(".fr-marker").length > 0 && (b.events.disableBlur(),
			b.selection.restore()),
			m([d]),
			!x[d])
				return !1;
			a(".fr-dropdown.fr-active").removeClass("fr-active").parent(".fr-toolbar").css("zIndex", ""),
			x[d].data("instance", b),
			...
			b.opts.toolbarInline && l && b.$tb && l.get(0) == b.$tb.get(0) && (c(d, b.opts.toolbarInline ? a(b.opts.scrollableContainer) : b.$box),
			h && (h = b.$tb.offset().top - b.helpers.getPX(b.$tb.css("margin-top"))),
			e && (e = b.$tb.offset().left + b.$tb.width() / 2),
			b.$tb.hasClass("fr-above") && h && (h += b.$tb.outerHeight()),
			i = 0),
			l = x[d].data("container"),
			!b.opts.iframe || i || k || (e && (e -= b.$iframe.offset().left),
			h && (h -= b.$iframe.offset().top)),
			e && (e -= j / 2),
			b.opts.toolbarBottom && l && b.$tb && l.get(0) == b.$tb.get(0) && (x[d].addClass("fr-above"),
			h && (h -= x[d].outerHeight())),
			...
			n && (0 == b.$el.find(".fr-marker").length && b.core.hasFocus() && b.selection.save(),
			b.events.disableBlur(),
			a(n).select().focus()),
		...
		function h(a) {
			x[a] && x[a].hasClass("fr-active") && (x[a].removeClass("fr-active fr-above"),
			b.events.trigger("popups.hide." + a),
			b.$tb && b.$tb.css("zIndex", ""),
			b.events.disableBlur(),
			x[a].find("input, textarea, button").filter(":focus").blur(),
			...
			return {
				...
				_inputKeydown: function(e) {
					...
					if (a.FE.KEYCODE.TAB == h) {
						...
						d.find(".fr-submit:visible").length > 0 && (e.preventDefault(),
						e.stopPropagation(),
						g.events.disableBlur(),
				...
				_preventFocus: function(c) {
					...
					!1) : void (f && a(f).is(g) && c.stopPropagation())
				},
				...
				_doPlaceholder: function(b) {
					var c = a(this).next();
					0 == c.length && a(this).after("<span>" + a(this).attr("placeholder") + "</span>"),
		...
		function u(c, d) {
			...
			b.events.$on(e, "keydown", "input, textarea, button, select", f._inputKeydown, !0),
	...
	a.FE.MODULES.refresh = function(b) {
		...
		function g(c) {
			...
			for (var d = b.selection.blocks(), e = 0; e < d.length; e++) {
				...
				if ("LI" == d[e].tagName || "LI" == d[e].parentNode.tagName)
					return c.removeClass("fr-disabled"),
					!0;
	...
	a.FE.MODULES.textEdit = function(b) {
		...
		function d() {
			...
			c = "INPUT" === b.$el.prop("tagName") ? b.$el.attr("placeholder") : b.$el.text(),
			d.find("input").val(c).trigger("change"),
			b.popups.setContainer("text.edit", a("body")),
			b.popups.show("text.edit", b.$el.offset().left + b.$el.outerWidth() / 2, b.$el.offset().top + b.$el.outerHeight(), b.$el.outerHeight())
		}
		function e() {
		...
		function f() {
			...
			"INPUT" === b.$el.prop("tagName") ? b.$el.attr("placeholder", c) : b.$el.text(c),
			b.events.trigger("contentChanged"),
			b.popups.hide("text.edit")
		}
		function g() {
			b.opts.editInPopup && (c(),
			e())
		}
		return {
			_init: g,
...
!function(a) {
	...
	a.FE.PLUGINS.codeView = function(b) {
		...
		function f(a, c) {
			k || i(),
			!l && b.opts.codeMirror && "undefined" != typeof CodeMirror && (l = CodeMirror.fromTextArea(k.get(0), b.opts.codeMirrorOptions)),
			b.undo.saveStep(),
			b.html.cleanEmptyTags(),
			...
			b.codeBeautifier && (d = b.codeBeautifier.run(d, {
				end_with_newline: !0,
				indent_inner_html: !0,
			...
			if (l) {
				...
				k.css("height", c),
				(b.opts.height || b.opts.heightMax) && k.css("max-height", b.opts.height || b.opts.heightMax),
				k.val(d.replace(/FROALA-SM/g, "").replace(/FROALA-EM/g, "")),
				k.focus(),
		...
		function g(a) {
			...
			if (a) {
				...
				b.$box.toggleClass("fr-code-view", !1),
				e(d)
		}
		function h() {
		...
		function i() {
			...
			b.opts.toolbarInline && (m = a('<a data-cmd="html" title="Code View" class="fr-command fr-btn html-switch' + (b.helpers.isMobile() ? "" : " fr-desktop") + '" role="button" tabindex="-1"><i class="fa fa-code"></i></button>'),
			b.$box.append(m),
	...
	a.FE.PLUGINS.lists = function(b) {
		...
		function f(c, d) {
			...
			for (var f = b.html.defaultTag(), g = 0; g < c.length; g++)
				"LI" != c[g].tagName && (f && c[g].tagName.toLowerCase() == f ? a(c[g]).replaceWith("<" + d + "><li" + b.node.attributes(c[g]) + ">" + a(c[g]).html() + "</li></" + d + ">") : a(c[g]).wrap("<" + d + "><li></li></" + d + ">"));
			b.clean.lists()
		}
		function g(e) {
			var f, g;
			...
			for (f = 0; f < e.length; f++) {
				...
				i.after(c("LI"))) : (b.node.isEmpty(i.get(0), !0) || 0 !== i.find(b.html.blockTagsQuery()).length || i.append("<br>"),
				i.append(c("LI")),
				i.prepend(d("LI"))),
				i.after(c(j.tagName)),
			...
			for (f = 0; f < h.length; f++) {
				...
				k.replaceWith(b.node.openTagString(k.get(0)) + l + b.node.closeTagString(k.get(0)))
			}
			b.$el.find("li:empty").remove(),
		...
		function i(a) {
			...
			b.selection.restore();
			for (var c = b.selection.blocks(), d = 0; d < c.length; d++)
				"LI" != c[d].tagName && "LI" == c[d].parentNode.tagName && (c[d] = c[d].parentNode);
			b.selection.save(),
			h(c, a) ? g(c) : f(c, a),
			b.html.unwrap(),
			b.selection.restore()
		...
		function j(c, d) {
			...
			if (e.get(0) != b.$el.get(0)) {
				...
				f && f.parentNode.tagName == d && b.$el.get(0).contains(f.parentNode) && c.addClass("fr-active")
			}
		}
		function k(c) {
			b.selection.save();
		...
		function m(a) {
			if ("indent" == a || "outdent" == a) {
				for (var c = !1, d = b.selection.blocks(), e = [], f = 0; f < d.length; f++)
					"LI" == d[f].tagName ? (c = !0,
					e.push(d[f])) : "LI" == d[f].parentNode.tagName && (c = !0,
					e.push(d[f].parentNode));
				c && ("indent" == a ? k(e) : l(e))
			}
		}
		function n() {
			...
			b.events.on("keydown", function(c) {
				if (c.which == a.FE.KEYCODE.TAB) {
					for (var d, e = b.selection.blocks(), f = [], g = 0; g < e.length; g++)
						"LI" == e[g].tagName ? (d = !0,
						f.push(e[g])) : "LI" == e[g].parentNode.tagName && (d = !0,
						f.push(e[g].parentNode));
					if (d)
						return c.preventDefault(),
						c.stopPropagation(),
						c.shiftKey ? l(f) : k(f),
...
// Links
// Remember that we've adjsuted this code to use [input-name], and not [name]
!function(e) {
	...
	e.FE.PLUGINS.link = function(t) {
		function n() {
			...
			if (!n && t.$wp) {
				var i = t.selection.element()
				  , r = t.selection.endElement();
				return "A" != i.tagName && (i = e(i).parents("a:first").get(0)),
				"A" != r.tagName && (r = e(r).parents("a:first").get(0)),
				r && r == i ? i : null 
			}
			return "A" == t.$el.get(0).tagName && t.core.hasFocus() ? t.$el : n && n.get(0).parentNode && "A" == n.get(0).parentNode.tagName ? n.get(0).parentNode : void 0
		}
		function i() {
			var e = t.image ? t.image.get() : null 
			  , n = [];
			if (e)
				"A" == e.get(0).parentNode.tagName && n.push(e.get(0).parentNode);
			else {
				var i, r, a, s;
				if (t.win.getSelection) {
					...
					if (l.getRangeAt && l.rangeCount) {
						s = t.doc.createRange();
						for (var o = 0; o < l.rangeCount; ++o)
							if (i = l.getRangeAt(o),
							r = i.commonAncestorContainer,
							r && 1 != r.nodeType && (r = r.parentNode),
							r && "a" == r.nodeName.toLowerCase())
								n.push(r);
							else {
								a = r.getElementsByTagName("a");
								for (var p = 0; p < a.length; ++p)
									s.selectNodeContents(a[p]),
									s.compareBoundaryPoints(i.END_TO_START, i) < 1 && s.compareBoundaryPoints(i.START_TO_END, i) > -1 && n.push(a[p])
							}
					}
				} else if (t.doc.selection && "Control" != t.doc.selection.type)
					if (i = t.doc.selection.createRange(),
					r = i.parentElement(),
					"a" == r.nodeName.toLowerCase())
						n.push(r);
					else {
						a = r.getElementsByTagName("a"),
						s = t.doc.body.createTextRange();
						for (var f = 0; f < a.length; ++f)
							s.moveToElementText(a[f]),
							s.compareEndPoints("StartToEnd", i) > -1 && s.compareEndPoints("EndToStart", i) < 1 && n.push(a[f])
					}
			}
			return n
		}
		function r(i) {
			...
			setTimeout(function() {
				if (!i || i && (1 == i.which || "mouseup" != i.type)) {
					var r = n()
					  , s = t.image ? t.image.get() : null ;
					if (r && !s) {
						if (t.image) {
							var l = t.node.contents(r);
							if (1 == l.length && "IMG" == l[0].tagName) {
								var o = t.selection.ranges(0);
								return 0 === o.startOffset && 0 === o.endOffset ? e(r).before(e.FE.MARKERS) : e(r).after(e.FE.MARKERS),
								t.selection.restore(),
								!1
							}
						}
		...
		function a(n) {
			...
			i || (i = l());
			var r = e(n);
			t.popups.isVisible("link.edit") || t.popups.refresh("link.edit"),
			t.popups.setContainer("link.edit", e(t.opts.scrollableContainer));
			var a = r.offset().left + e(n).outerWidth() / 2
			  , s = r.offset().top + r.outerHeight();
			t.popups.show("link.edit", a, s, r.outerHeight())
		}
		function s() {
			t.popups.hide("link.edit")
		}
		function l() {
			var e = "";
			t.opts.linkEditButtons.length > 1 && ("A" == t.$el.get(0).tagName && t.opts.linkEditButtons.indexOf("linkRemove") >= 0 && t.opts.linkEditButtons.splice(t.opts.linkEditButtons.indexOf("linkRemove"), 1),
			e = '<div class="fr-buttons">' + t.button.buildList(t.opts.linkEditButtons) + "</div>");
			var i = {
				buttons: e
			}
			  , r = t.popups.create("link.edit", i);
		...
		function p() {
			var i = t.popups.get("link.insert")
			  , r = n();
			if (r) {
				var a, s, l = e(r), o = i.find('input.fr-link-attr[type="text"]'), p = i.find('input.fr-link-attr[type="checkbox"]');
				for (a = 0; a < o.length; a++)
					s = e(o[a]),
					s.val(l.attr(s.attr("input-name") || ""));
				for (p.prop("checked", !1),
				a = 0; a < p.length; a++)
					s = e(p[a]),
					l.attr(s.attr("input-name")) == s.data("checked") && s.prop("checked", !0);
				i.find('input.fr-link-attr[type="text"][input-name="text"]').val(l.text())
			} else
				i.find('input.fr-link-attr[type="text"]').val(""),
				i.find('input.fr-link-attr[type="checkbox"]').prop("checked", !1),
				i.find('input.fr-link-attr[type="text"][input-name="text"]').val(t.selection.text());
			i.find("input.fr-link-attr").trigger("change");
			var f = t.image ? t.image.get() : null ;
			f ? i.find('.fr-link-attr[input-name="text"]').parent().hide() : i.find('.fr-link-attr[input-name="text"]').parent().show()
		}
		function f() {
			var n = t.$tb.find('.fr-command[data-cmd="insertLink"]')
			  , i = t.popups.get("link.insert");
			if (i || (i = d()),
			!i.hasClass("fr-active"))
				if (t.popups.refresh("link.insert"),
				t.popups.setContainer("link.insert", t.$tb || e(t.opts.scrollableContainer)),
				n.is(":visible")) {
					var r = n.offset().left + n.outerWidth() / 2
					  , a = n.offset().top + (t.opts.toolbarBottom ? 10 : n.outerHeight() - 10);
					t.popups.show("link.insert", r, a, n.outerHeight())
				} else
					t.position.forSelection(i),
					t.popups.show("link.insert")
		}
		function d(e) {
			if (e)
				return t.popups.onRefresh("link.insert", p),
				t.popups.onHide("link.insert", o),
				!0;
			var i = "";
			t.opts.linkInsertButtons.length >= 1 && (i = '<div class="fr-buttons">' + t.button.buildList(t.opts.linkInsertButtons) + "</div>");
			var r = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="10" height="10" viewBox="0 0 32 32"><path d="M27 4l-15 15-7-7-5 5 12 12 20-20z" fill="#FFF"></path></svg>'
			  , a = ""
			  , s = 0;
			a = '<div class="fr-link-insert-layer fr-layer fr-active" id="fr-link-insert-layer-' + t.id + '">',
			a += '<div class="fr-input-line"><input input-name="href" type="text" class="fr-link-attr" placeholder="URL" tabIndex="' + ++s + '"></div>',
			t.opts.linkText && (a += '<div class="fr-input-line"><input input-name="text" type="text" class="fr-link-attr" placeholder="' + t.language.translate("Text") + '" tabIndex="' + ++s + '"></div>');
			for (var l in t.opts.linkAttributes)
				if (t.opts.linkAttributes.hasOwnProperty(l)) {
					var f = t.opts.linkAttributes[l];
					a += '<div class="fr-input-line"><input input-name="' + l + '" type="text" class="fr-link-attr" placeholder="' + t.language.translate(f) + '" tabIndex="' + ++s + '"></div>'
				}
			t.opts.linkAlwaysBlank || (a += '<div class="fr-checkbox-line"><span class="fr-checkbox"><input input-name="target" class="fr-link-attr" data-checked="_blank" type="checkbox" id="fr-link-target-' + t.id + '" tabIndex="' + ++s + '"><span>' + r + '</span></span><label for="fr-link-target-' + t.id + '">' + t.language.translate("Open in new tab") + "</label></div>"),
			a += '<div class="fr-action-buttons"><button class="fr-command fr-submit" data-cmd="linkInsert" href="#" tabIndex="' + ++s + '" type="button">' + t.language.translate("Insert") + "</button></div></div>";
			var d = {
				buttons: i,
				input_layer: a
			}
			  , u = t.popups.create("link.insert", d);
			return t.$wp && t.events.$on(t.$wp, "scroll.link-insert", function() {
				var e = t.image ? t.image.get() : null ;
				e && t.popups.isVisible("link.insert") && E(),
				n && t.popups.isVisible("link.insert") && v()
			}),
			u
		}
		function u() {
			var i = n()
			  , r = t.image ? t.image.get() : null ;
			return t.events.trigger("link.beforeRemove", [i]) === !1 ? !1 : void (r && i ? (r.unwrap(),
			t.image.edit(r)) : i && (t.selection.save(),
		...
		function c() {
			t.events.on("keyup", function(t) {
				t.which != e.FE.KEYCODE.ESC && r(t)
			}),
			t.events.on("window.mouseup", r),
			t.events.$on(t.$doc, "selectionchange", r),
			d(!0),
			"A" == t.$el.get(0).tagName && t.$el.addClass("fr-view")
		}
		function k(n) {
			var i, r, a = t.opts.linkList[n], s = t.popups.get("link.insert"), l = s.find('input.fr-link-attr[type="text"]'), o = s.find('input.fr-link-attr[type="checkbox"]');
			for (r = 0; r < l.length; r++)
				i = e(l[r]),
				a[i.attr("input-name")] ? i.val(a[i.attr("input-name")]) : "text" != i.attr("input-name") && i.val("");
			for (r = 0; r < o.length; r++)
				i = e(o[r]),
				i.prop("checked", i.data("checked") == a[i.attr("input-name")])
		}
		function g() {
			var n, i, r = t.popups.get("link.insert"), a = r.find('input.fr-link-attr[type="text"]'), s = r.find('input.fr-link-attr[type="checkbox"]'), l = a.filter('[input-name="href"]').val(), o = a.filter('[input-name="text"]').val(), p = {};
			for (i = 0; i < a.length; i++)
				n = e(a[i]),
				["href", "text"].indexOf(n.attr("input-name")) < 0 && (p[n.attr("input-name")] = n.val());
			for (i = 0; i < s.length; i++)
				n = e(s[i]),
				n.is(":checked") ? p[n.attr("input-name")] = n.data("checked") : p[n.attr("input-name")] = n.data("unchecked");
			var f = e(t.o_win).scrollTop();
			m(l, o, p),
			e(t.o_win).scrollTop(f)
		}
		function h() {
			if (!t.selection.isCollapsed()) {
				...
				for (var n = t.$el.find(".fr-marker").addClass("fr-unprocessed").toArray(); n.length; ) {
					var i = e(n.pop());
					i.removeClass("fr-unprocessed");
					var r = t.node.deepestParent(i.get(0));
					if (r) {
						var a = i.get(0)
						  , s = ""
						  , l = "";
						do
							a = a.parentNode,
							t.node.isBlock(a) || (s += t.node.closeTagString(a),
							l = t.node.openTagString(a) + l);
						while (a != r);var o = t.node.openTagString(i.get(0)) + i.html() + t.node.closeTagString(i.get(0));
						i.replaceWith('<span id="fr-break"></span>');
						var p = e(r).html();
						p = p.replace(/<span id="fr-break"><\/span>/g, s + o + l),
						e(r).html(p)
					}
		...
		function m(a, s, l) {
			...
			var o = t.image ? t.image.get() : null ;
			o || "A" == t.$el.get(0).tagName ? "A" == t.$el.get(0).tagName && t.$el.focus() : (t.selection.restore(),
			t.popups.hide("link.insert"));
			var p = a;
			if (t.opts.linkConvertEmailAddress) {
				var f = /^[\w._]+@[a-zA-Z0-9_-]+?\.[a-zA-Z0-9]{2,3}$/i;
				f.test(a) && 0 !== a.indexOf("mailto:") && (a = "mailto:" + a)
			}
			if (0 === a.indexOf("tel:") || 0 === a.indexOf("sms:") || 0 === a.indexOf("mailto:") || 0 === a.indexOf("notes:") || 0 === a.indexOf("data:image") || "" === t.opts.linkAutoPrefix || /^(https?:|ftps?:|)\/\//.test(a) || (a = t.opts.linkAutoPrefix + a),
			a = t.helpers.sanitizeURL(a),
			t.opts.linkAlwaysBlank && (l.target = "_blank"),
			t.opts.linkAlwaysNoFollow && (l.rel = "nofollow"),
			s = s || "",
			a === t.opts.linkAutoPrefix) {
				var d = t.popups.get("link.insert");
				return d.find('input[input-name="href"]').addClass("fr-error"),
				t.events.trigger("link.bad", [p]),
				!1
			}
			var u, c = n();
			if (c) {
				u = e(c);
				var k = t.node.rawAttributes(c);
				for (var g in k)
					k.hasOwnProperty(g) && "class" != g && "style" != g && u.removeAttr(g);
				u.attr("href", a),
				s.length > 0 && u.text() != s && !o && u.text(s),
				o || u.prepend(e.FE.START_MARKER).append(e.FE.END_MARKER),
				u.attr(l),
				o || t.selection.restore()
			} else {
				o ? o.wrap('<a href="' + a + '"></a>') : (t.doc.execCommand("unlink", !1, !1),
				t.selection.isCollapsed() ? (s = 0 === s.length ? p : s,
				t.html.insert('<a href="' + a + '">' + e.FE.START_MARKER + s + e.FE.END_MARKER + "</a>"),
				t.selection.restore()) : s.length > 0 && s != t.selection.text() ? (t.selection.remove(),
				t.html.insert('<a href="' + a + '">' + e.FE.START_MARKER + s + e.FE.END_MARKER + "</a>"),
				t.selection.restore()) : (h(),
				t.doc.execCommand("createLink", !1, a)));
				for (var m = i(), v = 0; v < m.length; v++)
					u = e(m[v]),
					u.attr(l),
					u.removeAttr("_moz_dirty");
				1 == m.length && t.$wp && !o && (e(m[0]).prepend(e.FE.START_MARKER).append(e.FE.END_MARKER),
				t.selection.restore())
			}
			if (o) {
				var b = t.popups.get("link.insert");
		...
		function v() {
			...
			var i = n();
			if (i) {
				var r = t.popups.get("link.insert");
				r || (r = d()),
				t.popups.isVisible("link.insert") || (t.popups.refresh("link.insert"),
				t.selection.save(),
				t.helpers.isMobile() && (t.events.disableBlur(),
				t.$el.blur(),
				t.events.enableBlur())),
				t.popups.setContainer("link.insert", e(t.opts.scrollableContainer));
				var a = (t.image ? t.image.get() : null ) || e(i)
				  , l = a.offset().left + a.outerWidth() / 2
				  , o = a.offset().top + a.outerHeight();
				t.popups.show("link.insert", l, o, a.outerHeight())
			}
		}
		function b() {
			var e = t.image ? t.image.get() : null ;
			if (e)
				...
				t.events.enableBlur();
				var i = n();
				i && t.$wp ? (t.selection.restore(),
				s(),
				r()) : "A" == t.$el.get(0).tagName ? (t.$el.focus(),
				r()) : (t.popups.hide("link.insert"),
		...
		function x(i, a, s) {
			"undefined" == typeof s && (s = t.opts.linkMultipleStyles),
			"undefined" == typeof a && (a = t.opts.linkStyles);
			var l = n();
			if (!l)
				return !1;
			if (!s) {
	...
	e.FE.RegisterShortcut(75, "insertLink"),
	e.FE.RegisterCommand("insertLink", {
		title: "Insert Link",
		undo: !1,
		focus: !0,
		refreshOnCallback: !1,
		popup: !0,
		callback: function() {
			this.popups.isVisible("link.insert") ? (this.$el.find(".fr-marker") && (this.events.disableBlur(),
	...
	e.FE.RegisterCommand("linkOpen", {
		...
		refresh: function(e) {
			var t = this.link.get();
			t ? e.removeClass("fr-hidden") : e.addClass("fr-hidden")
		},
		callback: function() {
			var e = this.link.get();
			e && this.o_win.open(e.href)
		}
	}),
	...
	e.FE.RegisterCommand("linkBack", {
		...
		refresh: function(e) {
			var t = this.link.get()
			  , n = this.image ? this.image.get() : null ;
			n || t || this.opts.toolbarInline ? (e.removeClass("fr-hidden"),
	...
	e.FE.RegisterCommand("linkList", {
		...
		refreshAfterCallback: !1,
		html: function() {
			for (var e = '<ul class="fr-dropdown-list">', t = this.opts.linkList, n = 0; n < t.length; n++)
				e += '<li><a class="fr-command" data-cmd="linkList" data-param1="' + n + '">' + (t[n].displayText || t[n].text) + "</a></li>";
			return e += "</ul>"
		},
	...
	e.FE.RegisterCommand("linkInsert", {
		...
		refresh: function(e) {
			var t = this.link.get();
			t ? e.text(this.language.translate("Update")) : e.text(this.language.translate("Insert"))
		}
	}),
	...
	e.FE.RegisterCommand("imageLink", {
		...
		refresh: function(e) {
			var t, n = this.link.get();
			n ? (t = e.prev(),
			t.hasClass("fr-separator") && t.removeClass("fr-hidden"),
			e.addClass("fr-hidden")) : (t = e.prev(),
			t.hasClass("fr-separator") && t.addClass("fr-hidden"),
	...
	e.FE.RegisterCommand("linkStyle", {
		...
		html: function() {
			...
			for (var n in t)
				t.hasOwnProperty(n) && (e += '<li><a class="fr-command" data-cmd="linkStyle" data-param1="' + n + '">' + this.language.translate(t[n]) + "</a></li>");
			return e += "</ul>"
		},
		callback: function(e, t) {
		...
		refreshOnShow: function(t, n) {
			...
			if (i) {
				...
				n.find(".fr-command").each(function() {
					...
					e(this).toggleClass("fr-active", r.hasClass(t))
				})
			}
		}
	})
