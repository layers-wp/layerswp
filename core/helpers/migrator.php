...
class Layers_Widget_Migrator {
	...
	function get_preset_layouts() {
		...
		// new WP_Query for preset post types
	...
	public function page_sidebars_widgets( $post_object = NULL ) {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $widget_ids ) {
			...
			if ( !in_array( $sidebar_id, $valid_sidebars ) )
				continue;
	...
	public function import( $import_data = NULL ) {
		...
		foreach ( $import_data['widget_data'] as $sidebar_id => $sidebar_data ) {
			...
			if ( FALSE !== strpos( $sidebar_id, 'obox-layers-builder-' ) )
				$sidebar_id = 'obox-layers-builder-' . $import_data['post_id'];
	...
	public function do_ajax_create_preset() {
