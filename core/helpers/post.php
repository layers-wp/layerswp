...
if ( !function_exists( 'layers_get_the_author' ) ) {
	...
	function layers_get_the_author() {
		return sprintf( __( '<a href="%1$s" title="%2$s" rel="author">%3$s</a>', 'layerswp' ), esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ), esc_attr( sprintf( __( 'View all posts by %s', 'the-writer' ), get_the_author() ) ), esc_attr( get_the_author() )
		);
	}
	...
	if ( !function_exists( 'layers_backup_builder_pages' ) ) {
		...
		function layers_backup_builder_pages() {
			...
			if ( !isset( $_POST['pageid'] ) )
				wp_die( __( 'You shall not pass', 'layerswp' ) );
