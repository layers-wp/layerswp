...
if ( !function_exists( 'layers_get_header_class' ) ) {
	...
	function layers_get_header_class( $class = '' ) {
		...
		if (
			'#FFFFFF' == $light_or_dark && '' != $header_background_color_option
		) {
			$classes[] = 'invert';
		}
...
if ( !function_exists( 'layers_apply_inline_styles' ) ) {
	...
	function layers_apply_inline_styles() {
		...
		wp_enqueue_style(
			LAYERS_THEME_SLUG . '-inline-styles', get_template_directory_uri() . '/assets/css/inline.css'
		);

		wp_add_inline_style(
			LAYERS_THEME_SLUG . '-inline-styles', $layers_inline_css
		);
	}
