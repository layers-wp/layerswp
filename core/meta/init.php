	...
	*/
	...
	function inline_page_builder_button($actions,$post) {
		...
		if ( $can_edit_post && 'builder.php' == get_page_template_slug( $post->ID ) ) {
			$actions['builder'] = '<a href="' . admin_url() . 'customize.php?url=' . esc_url( get_the_permalink() ) . '&layers-builder=1" title="' . esc_attr( __( 'Edit Layout', LAYERS_THEME_SLUG ) ) . '">' . __( 'Build Page' ) . '</a>';
		}
