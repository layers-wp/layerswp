...
class Layers_Custom_Meta {
	...
	public function admin_enqueue_scripts() {
		...
		// Customizer general
		wp_enqueue_script(
			LAYERS_THEME_SLUG . '-admin-meta', get_template_directory_uri() . '/core/meta/js/meta.js', array(
			'backbone',
			'jquery',
			'wp-color-picker'
			), LAYERS_VERSION, true
	...
	public function page_builder_button_pointer( $pointers ) {
		...
		// This button is only used for pages
		if ( !in_array( $post->post_type, array( 'page' ) ) || 'publish' != get_post_status() )
			return;
