<div class="l_admin-browser">
	<div class="l_admin-products">
		...
		<?php foreach( $layers_migrator->get_preset_layouts() as $template_key => $template ) { ?>
			<div class="l_admin-product active  <?php echo ( isset( $template[ 'container-css' ] ) ?  esc_attr( $template[ 'container-css' ] ) : '' ); ?>" tabindex="0">
				...
				<label for="layers-preset-layout-<?php echo esc_attr( $template_key ); ?>-radio">
					...
					<div class="l_admin-product-actions">
						<a class="l_admin-button btn-primary customize load-customize" id="layers-generate-preset-layout-<?php echo esc_attr( $template_key ); ?>"  data-key="layers-preset-layout-<?php echo esc_attr( $template_key ); ?>"><?php _e( 'Select' , 'layerswp' ); ?></a>
					</div>
				</label>
			</div>
