...
if ( !class_exists( 'Layers_Widget' ) ) {
	...
	class Layers_Widget extends WP_Widget {
		...
		function get_widget_spacing_class( $widget = NULL ) {
			...
			if ( $this->check_and_return( $widget, 'design', 'advanced', 'margin-top' ) )
				$classes[] = 'margin-top-' . $this->check_and_return( $widget, 'design', 'advanced', 'margin-top' );
			if ( $this->check_and_return( $widget, 'design', 'advanced', 'margin-right' ) )
				$classes[] = 'margin-right-' . $this->check_and_return( $widget, 'design', 'advanced', 'margin-right' );
			if ( $this->check_and_return( $widget, 'design', 'advanced', 'margin-bottom' ) )
				$classes[] = 'margin-bottom-' . $this->check_and_return( $widget, 'design', 'advanced', 'margin-bottom' );
			if ( $this->check_and_return( $widget, 'design', 'advanced', 'margin-left' ) )
				$classes[] = 'margin-left-' . $this->check_and_return( $widget, 'design', 'advanced', 'margin-left' );

			if ( $this->check_and_return( $widget, 'design', 'advanced', 'padding-top' ) )
				$classes[] = 'padding-top-' . $this->check_and_return( $widget, 'design', 'advanced', 'padding-top' );
			if ( $this->check_and_return( $widget, 'design', 'advanced', 'padding-right' ) )
				$classes[] = 'padding-right-' . $this->check_and_return( $widget, 'design', 'advanced', 'padding-right' );
			if ( $this->check_and_return( $widget, 'design', 'advanced', 'padding-bottom' ) )
				$classes[] = 'padding-bottom-' . $this->check_and_return( $widget, 'design', 'advanced', 'padding-bottom' );
			if ( $this->check_and_return( $widget, 'design', 'advanced', 'padding-left' ) )
				$classes[] = 'padding-left-' . $this->check_and_return( $widget, 'design', 'advanced', 'padding-left' );
		...
		function get_custom_field_name( $widget_details = NULL, $level1 = '', $level2 = '', $field_name = '' ) {
			...
			if ( '' != $level1 )
				$final_field_name .= '[' . $level1 . ']';

			// Add second level of input string
			if ( '' != $level2 )
				$final_field_name .= '[' . $level2 . ']';
		...
		function get_custom_field_id( $widget_details = NULL, $level1 = '', $level2 = '', $field_id = '' ) {
			...
			// Add first level of input string
			if ( '' != $level1 )
				$final_field_id .= '-' . $level1;

			// Add second level of input string
			if ( '' != $level2 )
				$final_field_id .= '-' . $level2;
