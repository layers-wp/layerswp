...
if ( !class_exists( 'Layers_Content_Widget' ) ) {
	...
	class Layers_Content_Widget extends Layers_Widget {
		...
		function enqueue_scripts() {
			...
			wp_enqueue_script(
				LAYERS_THEME_SLUG . '-layers-masonry-js', get_template_directory_uri() . '/assets/js/layers.masonry.js', array(
				'jquery'
				)
					...
					<div class="row <?php echo $this->get_widget_layout_class( $widget ); ?> <?php echo $this->check_and_return( $widget, 'design', 'liststyle' ); ?>">
						...
						foreach ( explode( ',', $widget['column_ids'] ) as $column_key ) {
							...
							$media = layers_get_feature_media(
								$featureimage, $use_image_ratio, $featurevideo
							);
		...
		function form( $instance ) {
			...
			$components = apply_filters( 'layerswp_content_widget_design_bar_components', array(
				'layout',
				'custom',
				'fonts',
				'background',
			...
			$custom_components = apply_filters( 'layerswp_content_widget_design_bar_custom_components', array(
				'liststyle' => array(
			...
			<div class="layers-container-large" id="layers-column-widget-<?php echo $this->number; ?>">
				...
				<section class="layers-accordion-section layers-content">
					...
					<?php
					// If we have some columns, let's break out their IDs into an array
					if ( isset( $column_ids ) && '' != $column_ids )
						$columns = explode( ',', $column_ids );
					?>
					...
					<ul id="column_list_<?php echo esc_attr( $this->number ); ?>" class="layers-accordions layers-accordions-sortable layers-sortable" data-id_base="<?php echo $this->id_base; ?>" data-number="<?php echo $this->number; ?>">
						<?php if ( isset( $columns ) && is_array( $columns ) ) { ?>
							<?php
							foreach ( $columns as $columnguid ) {
								$this->column_item( array(
									'id_base' => $this->id_base,
									'number' => $this->number
									), $columnguid, ( isset( $instance['columns'][$columnguid] ) ) ? $instance['columns'][$columnguid] : NULL  );
							}
							?>
						<?php } ?>
					</ul>
...
// Form
		...
		function column_item( $widget_details = array(), $column_guid = NULL, $instance = NULL ) {
			...
			if ( !isset( $column_guid ) )
				$column_guid = rand( 1, 1000 );
