 ...
 * @since Layers 1.0.0
 */
if( !class_exists( 'Layers_Test_Hide_Show_Widget' ) && class_exists('Layers_Widget') ) {
	class Layers_Test_Hide_Show_Widget extends Layers_Widget {

		/**
		*  Widget construction
		*/
		function Layers_Test_Hide_Show_Widget(){
			...
			*
		 	...
		 	*/
			$this->widget_title = __( 'Test Hide / Show' , 'layerswp' );
			$this->widget_id = 'test-hide-show';
			$this->post_type = '';
			$this->taxonomy = '';
		...
		function widget( $args, $instance ) {
			...
			*/
			...
			<section id="<?php echo esc_attr( $widget_id ); ?>" class="<?php echo esc_attr( $widget_container_class ); ?>" <?php echo $this->custom_anchor( $widget ); ?>>
				...
				<?php do_action( 'layers_after_content_widget_inner', $this, $widget ); ?>
				...
				<section class="layers-accordion-section layers-content">
					<p class="layers-form-item">
						<?php echo $this->form_elements()->input(
							array(
								'type' => 'number',
								'name' => $this->get_layers_field_name( 'field-1' ) ,
								'id' => $this->get_layers_field_id( 'field-1' ) ,
								'placeholder' => __( 'Add 20 to show next field' , 'layerswp' ),
								'value' => ( isset( $widget['field-1'] ) ) ? $widget['field-1'] : NULL,
					...
					<p class="layers-form-item">
						<?php echo $this->form_elements()->input(
							array(
								'type' => 'select',
								'name' => $this->get_layers_field_name( 'field-2' ) ,
								'id' => $this->get_layers_field_id( 'field-2' ) ,
								'label' => __( 'Hide "field-3" only for "option-2"' , 'layerswp' ),
								'value' => ( isset( $widget['field-2'] ) ) ? $widget['field-2'] : NULL,
								'class' => 'layers-text layers-large',
								'options' => array(
									'option-1' => __( 'Select "option-2" to show next field', 'surror' ),
									'option-2' => __( 'Option 2', 'surror' ),
									'option-3' => __( 'Option 3', 'surror' ),
								),
								'data' => array(
									'show-if-selector' => '#' . $this->get_layers_field_id( 'field-1' ),
									'show-if-operator' => '==',
									'show-if-value' => '20',
								),
							)
					...
					<p class="layers-form-item">
						<label>Tick to SHOW 4th field</label>
						<?php echo $this->form_elements()->input(
							array(
								'type' => 'checkbox',
								'name' => $this->get_layers_field_name( 'field-3' ) ,
								'id' => $this->get_layers_field_id( 'field-3' ) ,
								'value' => ( isset( $widget['field-3'] ) ) ? $widget['field-3'] : NULL,
								'class' => 'layers-text layers-large',
								'data' => array(
									'show-if-selector' => '#' . $this->get_layers_field_id( 'field-2' ),
									'show-if-operator' => '!=',
									'show-if-value' => 'option-1',
								),
							)
						); ?>
					</p>
					<p class="layers-form-item">
						<?php echo $this->form_elements()->input(
							array(
								'type' => 'text',
								'name' => $this->get_layers_field_name( 'field-4' ) ,
								'id' => $this->get_layers_field_id( 'field-4' ) ,
								'placeholder' => __( 'Yep...! Succeed...! :)' , 'layerswp' ),
								'label' => __( 'Check to show last field' , 'layerswp' ),
								'value' => ( isset( $widget['field-4'] ) ) ? $widget['field-4'] : NULL,
								'class' => 'layers-text layers-large',
								'data' => array(
									'show-if-selector' => '#' . $this->get_layers_field_id( 'field-3' ),
									'show-if-operator' => '==',
									'show-if-value' => 'true',
								),
							)
	...
	// Add our function to the widgets_init hook.
	 register_widget("Layers_Test_Hide_Show_Widget");
}